# DeleteAllPending — User Guide

## Description

Deletes all pending orders by type.

## Installation

1. Copy the entire `DeleteAllPending/` folder to your MT5 `MQL5/Scripts/` directory
2. Open MetaEditor (F4 in MT5 terminal)
3. Navigate to the `DeleteAllPending/` folder
4. Compile `DeleteAllPending_v0.0.4.mq5` (F7)
5. Drag the script from MT5 Navigator onto any chart

## Usage

1. Drag DeleteAllPending onto a chart
2. The script executes immediately: **Delete Pending Orders**
...
ML data is stored in `MQL5/Files/DeleteAllPending/`:
- `exec_log.csv` — Execution journal

## Testing

Run the test suite:
1. Compile `Tests/DeleteAllPending_TestSuite_v0.0.4.mq5` in MetaEditor
