# ScalpingEA — Architecture

## Overview

Fast in-and-out scalping with spread filter. Part of the PutraWorks MetaTrader-AI collection.

## Tool-Specific Parameters

- `scalpTP` — configured in Config_v0.0.4.mqh
- `scalpSL` — configured in Config_v0.0.4.mqh
- `maxSpread` — configured in Config_v0.0.4.mqh
- `rsiPeriod` — configured in Config_v0.0.4.mqh

## ML Pattern Categories

- `OversoldBounce` — tracked and scored by PatternRecognition_v0.0.4.mqh
- `OverboughtDrop` — tracked and scored by PatternRecognition_v0.0.4.mqh
- `SpreadSqueeze` — tracked and scored by PatternRecognition_v0.0.4.mqh
- `MicroTrend` — tracked and scored by PatternRecognition_v0.0.4.mqh

## Module Architecture

```
┌─────────────────────────────────────────────────────────┐
│                    ScalpingEA_v0.0.4.mq5                      │
│                   (Main EA Entry Point)                   │
├──────────┬──────────┬──────────┬──────────┬─────────────┤
...
| Module | File | Purpose |
|--------|------|---------|
| Config | Config_v0.0.4.mqh | ScalpingEA-specific parameter profiles with scalpTP, scalpSL, maxSpread, rsiPeriod |
...
| LearningEngine | LearningEngine_v0.0.4.mqh | Post-trade lesson extraction for OversoldBounce, OverboughtDrop, SpreadSqueeze, MicroTrend |
| PatternRecognition | PatternRecognition_v0.0.4.mqh | Scores OversoldBounce, OverboughtDrop, SpreadSqueeze, MicroTrend patterns by win rate |
| StrategyEvolution | StrategyEvolution_v0.0.4.mqh | Multi-profile management with promotion/retirement |
| OptimizationEngine | OptimizationEngine_v0.0.4.mqh | Adaptive tuning of scalpTP, scalpSL, maxSpread, rsiPeriod |
...
```
ScalpingEA/
├── ScalpingEA_v0.0.4.mq5              # Main file — compile this
├── Include/
│   ├── ScalpingEA_v0.0.4.mqh           # Core EA logic
│   ├── Config_v0.0.4.mqh           # ScalpingEA parameters
...
├── Tests/ScalpingEA_TestSuite_v0.0.4.mq5
├── docs/
...
└── Archive/                       # Previous versions (read-only)
    ├── ScalpingEA_v0.0.1.mq5
    ├── ScalpingEA_v0.0.2.mq5
    └── ScalpingEA_v0.0.3.mq5
```
