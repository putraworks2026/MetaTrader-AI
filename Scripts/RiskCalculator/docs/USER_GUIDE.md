# RiskCalculator — User Guide

## Description

Calculates lot size from risk % and SL distance.

## Installation

1. Copy the entire `RiskCalculator/` folder to your MT5 `MQL5/Scripts/` directory
2. Open MetaEditor (F4 in MT5 terminal)
3. Navigate to the `RiskCalculator/` folder
4. Compile `RiskCalculator_v0.0.4.mq5` (F7)
5. Drag the script from MT5 Navigator onto any chart

## Usage

1. Drag RiskCalculator onto a chart
2. The script executes immediately: **Calculate Lot Size**
...
ML data is stored in `MQL5/Files/RiskCalculator/`:
- `exec_log.csv` — Execution journal

## Testing

Run the test suite:
1. Compile `Tests/RiskCalculator_TestSuite_v0.0.4.mq5` in MetaEditor
