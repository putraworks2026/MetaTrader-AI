# LiquidityZones — User Guide

## Description

Detects equal highs/lows as liquidity pools.

## Installation

1. Copy the entire `LiquidityZones/` folder to your MT5 `MQL5/Indicators/` directory
2. Open MetaEditor (F4 in MT5 terminal)
3. Navigate to the `LiquidityZones/` folder
4. Compile `LiquidityZones_v0.0.4.mq5` (F7)
...
- Which market regimes favor Liquidity Sweep signals
...
ML data is stored in `MQL5/Files/LiquidityZones/`:
...
Run the test suite:
1. Compile `Tests/LiquidityZones_TestSuite_v0.0.4.mq5` in MetaEditor
