...
2. **Copy the EA**: Place `ScalpingEA.mq5` in `MQL5/Experts/`
3. **Copy includes**: Place the entire `ScalpingEA/` folder in `MQL5/Include/`
4. **Compile**: Open MetaEditor (F4), open `ScalpingEA.mq5`, press F7
5. **Verify**: Check the Errors tab — should show 0 errors, 0 warnings
6. **Attach**: Drag ScalpingEA from the Navigator panel onto a chart
...
All EA data is stored in `MQL5/Files/ScalpingEA/`:
...
2. Select ScalpingEA as the Expert
