//+------------------------------------------------------------------+
//| SignalPatterns_v0.0.4.mqh — AutoSupportResistance Signal Pattern Scoring
//| Copyright 2026, PutraWorks
//| Scores which conditions produce good S/R Level Touch signals
//+------------------------------------------------------------------+
#ifndef AUTOSUPPORTRESISTANCE_SIGNAL_PATTERNS_MQH
#define AUTOSUPPORTRESISTANCE_SIGNAL_PATTERNS_MQH
...
public:
   ...
   string GetBestCondition() { if(m_count==0) return "No data for S/R Level Touch"; int b=0; for(int i=1;i<m_count;i++) if(m_patterns[i].score>m_patterns[b].score) b=i; return m_patterns[b].condition+" | SR: "+DoubleToString(m_patterns[b].successRate*100,1)+"%"; }
   int GetPatternCount() { return m_count; }
};

#endif // AUTOSUPPORTRESISTANCE_SIGNAL_PATTERNS_MQH
