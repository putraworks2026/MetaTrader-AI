# VolumeProfile — User Guide

## Description

Volume at price levels with POC and value area.

## Installation

1. Copy the entire `VolumeProfile/` folder to your MT5 `MQL5/Indicators/` directory
2. Open MetaEditor (F4 in MT5 terminal)
3. Navigate to the `VolumeProfile/` folder
4. Compile `VolumeProfile_v0.0.4.mq5` (F7)
...
- Which market regimes favor POC Touch signals
...
ML data is stored in `MQL5/Files/VolumeProfile/`:
...
Run the test suite:
1. Compile `Tests/VolumeProfile_TestSuite_v0.0.4.mq5` in MetaEditor
