...
Exports trade history to CSV with custom columns. Part of the PutraWorks MetaTrader-AI collection.

## Action Type

This script performs: **Export to CSV**

## ML Module Architecture

```
┌─────────────────────────────────────────────────────────┐
│                  ExportTradeHistory_v0.0.4.mq5                       │
...
| ExecJournal | ExecJournal_v0.0.4.mqh | Logs each Export to CSV execution with duration, items processed, result |
...
```
ExportTradeHistory/
├── ExportTradeHistory_v0.0.4.mq5              # Main file — compile this
├── Include/
│   ├── ExportTradeHistory_v0.0.4.mqh           # Core script logic
│   ├── ExecConfig_v0.0.4.mqh      # Execution config
│   └── ExecJournal_v0.0.4.mqh     # Execution journal
├── Tests/ExportTradeHistory_TestSuite_v0.0.4.mq5
├── docs/
│   ├── ARCHITECTURE.md
│   ├── USER_GUIDE.md
│   └── CHANGELOG.md
...
└── Archive/                       # Previous versions (read-only)
    ├── ExportTradeHistory_v0.0.1.mq5
    ├── ExportTradeHistory_v0.0.2.mq5
    └── ExportTradeHistory_v0.0.3.mq5
```
