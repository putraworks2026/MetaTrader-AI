...
2. **Copy the EA**: Place `GridTradingEA.mq5` in `MQL5/Experts/`
3. **Copy includes**: Place the entire `GridTradingEA/` folder in `MQL5/Include/`
4. **Compile**: Open MetaEditor (F4), open `GridTradingEA.mq5`, press F7
5. **Verify**: Check the Errors tab — should show 0 errors, 0 warnings
6. **Attach**: Drag GridTradingEA from the Navigator panel onto a chart
...
All EA data is stored in `MQL5/Files/GridTradingEA/`:
...
2. Select GridTradingEA as the Expert
