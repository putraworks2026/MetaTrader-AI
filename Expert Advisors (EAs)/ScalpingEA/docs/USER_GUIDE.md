# ScalpingEA — User Guide

## Description

Fast in-and-out scalping with spread filter.

## Installation

1. Copy the entire `ScalpingEA/` folder to your MT5 `MQL5/Experts/` directory
2. Open MetaEditor (F4 in MT5 terminal)
3. Navigate to the `ScalpingEA/` folder
4. Compile `ScalpingEA_v0.0.4.mq5` (F7)
...
|-----------|---------|-------------|
| scalpTP | See Config | scalpTP |
| scalpSL | See Config | scalpSL |
| maxSpread | See Config | maxSpread |
| rsiPeriod | See Config | rsiPeriod |
...
### Self-Improvement
ScalpingEA learns from every trade:
- **Pattern Recognition**: Tracks OversoldBounce, OverboughtDrop, SpreadSqueeze, MicroTrend and scores them by win rate
- **Learning Engine**: Extracts lessons from wins and losses (e.g., "OversoldBounce succeeds in trending markets")
...
ML data is stored in `MQL5/Files/ScalpingEA/`:
...
Run the test suite:
1. Compile `Tests/ScalpingEA_TestSuite_v0.0.4.mq5` in MetaEditor
