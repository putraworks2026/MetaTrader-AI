//+------------------------------------------------------------------+
//| SignalPatterns_v0.0.4.mqh — FibonacciAutoDraw Signal Pattern Scoring
//| Copyright 2026, PutraWorks
//| Scores which conditions produce good Fib Retracement Touch signals
//+------------------------------------------------------------------+
#ifndef FIBONACCIAUTODRAW_SIGNAL_PATTERNS_MQH
#define FIBONACCIAUTODRAW_SIGNAL_PATTERNS_MQH
...
public:
   ...
   string GetBestCondition() { if(m_count==0) return "No data for Fib Retracement Touch"; int b=0; for(int i=1;i<m_count;i++) if(m_patterns[i].score>m_patterns[b].score) b=i; return m_patterns[b].condition+" | SR: "+DoubleToString(m_patterns[b].successRate*100,1)+"%"; }
   int GetPatternCount() { return m_count; }
};

#endif // FIBONACCIAUTODRAW_SIGNAL_PATTERNS_MQH
