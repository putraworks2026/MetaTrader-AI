//+------------------------------------------------------------------+
//| SignalLearning_v0.0.4.mqh — OrderBlocks Signal Reliability Learning
//| Copyright 2026, PutraWorks
//| Learns which OB Mitigation signals are most reliable
//+------------------------------------------------------------------+
#ifndef ORDERBLOCKS_SIGNAL_LEARNING_MQH
#define ORDERBLOCKS_SIGNAL_LEARNING_MQH
...
public:
   ...
   // OrderBlocks-specific: OB Mitigation
   void AnalyzeSignal(const SignalEntry &se)
   {
      if(se.confidence>70 && se.outcome==SIGNAL_SUCCESS) AddLesson("HighConfidence","High-confidence OB Mitigation signals succeed",1.0);
      else if(se.confidence>70 && se.outcome==SIGNAL_FAILED) AddLesson("HighConfidence","High-confidence OB Mitigation FAILED - review threshold",-1.0);
      if(se.outcome==SIGNAL_FAILED && (se.hour<7 || se.hour>20)) AddLesson("OffHours","OB Mitigation signals fail in off-hours",-0.5);
      if(se.regime==REGIME_VOLATILE && se.outcome==SIGNAL_FAILED) AddLesson("Volatile","OB Mitigation signals fail in volatile regime",-0.8);
      if(se.regime==REGIME_TRENDING && se.outcome==SIGNAL_SUCCESS) AddLesson("Trending","OB Mitigation signals succeed in trending regime",0.7);
      if(se.quality==SIGNAL_QUALITY_HIGH && se.outcome==SIGNAL_FAILED) AddLesson("QualityMismatch","High-quality OB Mitigation failed - recalibrate",-0.9);
   }
   ...
   string GetTopInsight() { if(m_count==0) return "No insights yet for OB Mitigation"; int b=0; for(int i=1;i<m_count;i++) if(MathAbs(m_lessons[i].impact)*m_lessons[i].occurrences>MathAbs(m_lessons[b].impact)*m_lessons[b].occurrences) b=i; return m_lessons[b].condition+": "+m_lessons[b].description; }
...
#endif // ORDERBLOCKS_SIGNAL_LEARNING_MQH
