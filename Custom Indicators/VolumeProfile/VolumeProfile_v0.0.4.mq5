...
void ML_Init()
{
    g_signalJournal.Init("VolumeProfile");
    g_signalLearning.Init("VolumeProfile");
    g_signalDashboard.Init("VolumeProfile");
    Print("[ML] VolumeProfile signal engine initialized");
}
...
{
    g_signalDashboard.Update("POC Touch", g_signalJournal.GetCount(), 0, 0, 0.0,
...
void ML_OnDeinit()
{
    ...
    Print("[ML] VolumeProfile signal engine shutdown");
}
...
void OnDeinit(const int reason) { ML_OnDeinit(); }
