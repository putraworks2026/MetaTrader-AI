...
2. **Copy the EA**: Place `SetBreakevenAll.mq5` in `MQL5/Experts/`
3. **Copy includes**: Place the entire `SetBreakevenAll/` folder in `MQL5/Include/`
4. **Compile**: Open MetaEditor (F4), open `SetBreakevenAll.mq5`, press F7
5. **Verify**: Check the Errors tab — should show 0 errors, 0 warnings
6. **Attach**: Drag SetBreakevenAll from the Navigator panel onto a chart
...
All EA data is stored in `MQL5/Files/SetBreakevenAll/`:
...
2. Select SetBreakevenAll as the Expert
