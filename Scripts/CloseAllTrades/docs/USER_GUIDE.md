...
2. **Copy the EA**: Place `CloseAllTrades.mq5` in `MQL5/Experts/`
3. **Copy includes**: Place the entire `CloseAllTrades/` folder in `MQL5/Include/`
4. **Compile**: Open MetaEditor (F4), open `CloseAllTrades.mq5`, press F7
5. **Verify**: Check the Errors tab — should show 0 errors, 0 warnings
6. **Attach**: Drag CloseAllTrades from the Navigator panel onto a chart
...
All EA data is stored in `MQL5/Files/CloseAllTrades/`:
...
2. Select CloseAllTrades as the Expert
