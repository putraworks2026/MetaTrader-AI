...
2. **Copy the EA**: Place `AutoSupportResistance.mq5` in `MQL5/Experts/`
3. **Copy includes**: Place the entire `AutoSupportResistance/` folder in `MQL5/Include/`
4. **Compile**: Open MetaEditor (F4), open `AutoSupportResistance.mq5`, press F7
5. **Verify**: Check the Errors tab — should show 0 errors, 0 warnings
6. **Attach**: Drag AutoSupportResistance from the Navigator panel onto a chart
...
All EA data is stored in `MQL5/Files/AutoSupportResistance/`:
...
2. Select AutoSupportResistance as the Expert
